...
All dependencies are included in the development container.
...
to perform a complete build and check of u-os-hub-api.
...
We use Spectral for both OpenAPI and AsyncAPI linting based on the rules in `tools/spectral`.
Run all repository checks with `just check`, or a single scope with commands such as `just openapi::check`.
...
Before creating a merge request, run `just check` to validate your changes. This will prevent failures in the CI pipeline later.
